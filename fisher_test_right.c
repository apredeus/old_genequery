...
"order).\n"
  ...
  while (fgets(buf, MAXLINELEN, test_file)) {
    ...
    if (sscanf(bufptr, "%s %u %u %u %u %u %u", idstr, &m11, &m12, &m21, &m22, &m31, &m32) < 7) {
      ...
      //printf("two-sided p-value for %s: %e, logp: %.3f\n", idstr, fisher22(m11, m12, m21, m22), log10(fisher22(m11, m12, m21, m22)));
      //printf("left      p-value for %s: %e, logp: %.3f\n", idstr, fisher22_1sided(m11, m12, m21, m22, 1),log10(fisher22_1sided(m11, m12, m21, m22, 1)));
      //printf("right     p-value for %s: %e, logp: %.3f\n", idstr, fisher22_1sided(m11, m12, m21, m22, 0),log10(fisher22_1sided(m11, m12, m21, m22, 0)));
      // I only want one value, 1-sided Fisher's test of "greater" overlap. -- A.P.
      printf("%e\t%.3f\n",fisher22_1sided(m11, m12, m21, m22, 1),log10(fisher22_1sided(m11, m12, m21, m22, 1)));
    } else {
